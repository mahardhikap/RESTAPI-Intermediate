...
const sortRecipe = async (post) => {
    return new Promise((resolve, reject) => {
        ...
        pool.query(`SELECT recipe.id, recipe.users_id, recipe.title, recipe.image, recipe.ingredients, category.name AS category, users.username, users.photo, recipe.created_at FROM recipe JOIN category ON recipe.category_id = category.id JOIN users ON recipe.users_id = users.id ORDER BY ${sortby} ${sort} OFFSET ${offset} LIMIT ${limit}`, (err, results) => {
          if (!err) {
            const data = {
